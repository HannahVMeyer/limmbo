...
def multiple_set_covers_all(number_of_traits, sample_size_ori, number_of_covers, seed=2152):
	...
	for i in range(0,len(bootstrap_array)):
		while (len(bootstrap_array[i])!=maxlen):
			bootstrap_array[i] = np.unique(np.append(rand_state.choice(a=list(range(number_of_traits)), size=(maxlen-len(bootstrap_array[i])), replace=False),bootstrap_array[i]))	

...
class GWAS(object):
    ...
    def runAssociationAnalysis(self, mode, setup="lmm", adjustSingleTrait=None):
		...
		>>> gwas = GWAS(datainput=indata, seed=10, verbose=False)
                ...
                (1, 20)
                >>> resultsAssociation['pvalues'].min()
	...
	return {"pvalues": pvalues, "pvalues_adjust": pvalues_adjust, 
		"betas": betas}
    ...
    def __computeFDR(self):
        verboseprint("Computing empirical FDR")
        np.random.seed(seed=self.seed)
    ...
    def __varianceDecomposition(self, cache=True):
	    ...
	    vd = MTSet(Y=self.phenotypes, XX=self.relatedness)
